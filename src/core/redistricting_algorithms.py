...
def _weighted_partisan_share(gdf):
    """Returns the population-weighted partisan share for a partition."""
...
class RedistrictingAlgorithm:
    ...
    def __init__(self, state_data, num_districts, population_equality_weight=1.0, compactness_weight=1.0,
        ...
        if 'partisan_score' in self.state_data.columns:
            self.state_data['partisan_score'] = pd.to_numeric(
                self.state_data['partisan_score'], errors='coerce'
            ).fillna(0.5)
        else:
            self.state_data['partisan_score'] = 0.5
